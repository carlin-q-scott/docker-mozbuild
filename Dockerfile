...
RUN Invoke-WebRequest `
     ...
     -UseBasicParsing -OutFile MozillaBuildSetup-Latest.exe;
RUN .\MozillaBuildSetup-Latest.exe /S
# returns immediately so don't try to remove the exe

ADD bash_profile .bash_profile
RUN mkdir \mozilla-source
VOLUME C:\mozilla-source

# Bug: the subshell is failing to run
SHELL ["cmd", "/S", "/C", "C:\\mozilla-build\\start-shell.bat"]
RUN pip install Mercurial
ADD init-mach.sh init-mach.sh
RUN init-mach.sh

WORKDIR "C:\\mozilla-build\\"

# Default to PowerShell if no other command specified.
ENTRYPOINT [ "C:\\mozilla-build\\msys\\bin\\bash.exe", "-c", "sshd" ]